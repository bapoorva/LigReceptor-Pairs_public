...
server <- function(input, output,session) {
  ...
  ligrecpairs = reactive({
    ...
    if(is.null(lig_avg)==F){rl=left_join(rl,lig_avg,by=c("Ligand.ApprovedSymbol"="SYMBOL")) }
    if(is.null(rec_avg)==F){rl=left_join(rl,rec_avg,by=c("Receptor.ApprovedSymbol"="SYMBOL"))}
